 ...
 * @returns {string} Package name like "opencode-matrixx-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `opencode-matrixx-${os}-${arch}${suffix}`;
}
 ...
 * @returns {string} Relative path like "opencode-matrixx-darwin-arm64/bin/matrixx"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/matrixx${ext}`;
}
