...
import type { PaneAction, WindowState } from '../../../src/features/tmux-subagent/types'
...
mock.module('../../../src/features/tmux-subagent/pane-state-querier', () => ({
...
mock.module('../../../src/features/tmux-subagent/action-executor', () => ({
  executeActions: mockExecuteActions,
  executeAction: mockExecuteAction,
  executeActionWithDeps: mockExecuteAction,
}))

mock.module('../../../src/shared/tmux', () => {
  const { isInsideTmux, getCurrentPaneId } = require('../../../src/shared/tmux/tmux-utils')
  const { POLL_INTERVAL_BACKGROUND_MS, SESSION_TIMEOUT_MS, SESSION_MISSING_GRACE_MS } = require('../../../src/shared/tmux/constants')
  return {
    isInsideTmux,
    getCurrentPaneId,
...
describe('TmuxSessionManager', () => {
  ...
  describe('constructor', () => {
    test('enabled when config.enabled=true and isInsideTmux=true', async () => {
      ...
      const { TmuxSessionManager } = await import('../../../src/features/tmux-subagent/manager')
...
describe('DecisionEngine', () => {
  describe('calculateCapacity', () => {
    test('calculates correct 2D grid capacity', async () => {
      // given
      const { calculateCapacity } = await import('../../../src/features/tmux-subagent/decision-engine')
  ...
  describe('decideSpawnActions', () => {
    test('returns spawn action with splitDirection when under capacity', async () => {
      // given
      const { decideSpawnActions } = await import('../../../src/features/tmux-subagent/decision-engine')
