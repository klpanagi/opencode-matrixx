...
describe("executeSyncContinuation - toast cleanup error paths", () => {
  ...
  beforeEach(() => {
    ...
    const { __setTimingConfig } = require("../../../src/tools/delegate-task/timing")
    __setTimingConfig({
      POLL_INTERVAL_MS: 10,
    ...
    const { initTaskToastManager, _resetTaskToastManagerForTesting } = require("../../../src/features/task-toast-manager/manager")
  ...
  afterEach(() => {
    ...
    const { __resetTimingConfig } = require("../../../src/tools/delegate-task/timing")
    __resetTimingConfig()
  ...
  test("removes toast when fetchSyncResult throws", async () => {
    ...
    const { executeSyncContinuation } = require("../../../src/tools/delegate-task/sync-continuation")
  ...
  test("no crash when toastManager is null", async () => {
    ...
    const { _resetTaskToastManagerForTesting } = require("../../../src/features/task-toast-manager/manager")
    _resetTaskToastManagerForTesting()
