...
import { findWorkspaceRoot } from "../../../src/tools/lsp/lsp-client-wrapper"

describe("lsp utils", () => {
  describe("findWorkspaceRoot", () => {
    it("returns an existing directory even when the file path points to a non-existent nested path", () => {
      const tmp = mkdtempSync(join(os.tmpdir(), "matrixx-lsp-root-"))
      try {
    ...
    it("prefers the nearest marker directory when markers exist above the file", () => {
      const tmp = mkdtempSync(join(os.tmpdir(), "matrixx-lsp-marker-"))
      try {
