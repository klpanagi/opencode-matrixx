...
describe("ulw bootstrap", () => {
  // Superseded by 5a32d0c9b ("stop false task-continuation bootstrap on research
  // sessions"): bg-task history alone must NOT trigger a continuation bootstrap.
  test("does not bootstrap when task dir is absent even with bg-task history", async () => {
    ...
    expect(store.getState("ulw1").countdownTimer).toBeUndefined()
    store.shutdown()
  ...
  test("does not bootstrap when tasks dir is empty even with bg-task history", async () => {
    const dir = mkdtempSync(join(tmpdir(), "ulw-boot-"))
  ...
  test("does not bootstrap when explorer and worker bg tasks mix", async () => {
    const dir = mkdtempSync(join(tmpdir(), "ulw-boot-"))
    const toastMock = mock(async () => ({} as never))
    const ctx = {
      directory: dir,
