...
import { _resetForTesting, subagentSessions } from "../../../src/features/session-state"
...
mock.module("../../../src/features/hook-message-injector/constants", () => ({
  OPENCODE_STORAGE: TEST_STORAGE_ROOT,
  MESSAGE_STORAGE: TEST_MESSAGE_STORAGE,
  PART_STORAGE: TEST_PART_STORAGE,
}))

mock.module("../../../src/shared/opencode-message-dir", () => ({
...
describe("architect hook", () => {
  ...
  describe("tool.execute.after handler", () => {
    ...
    test("should ignore non-task tools", async () => {
     ...
     test("should append session ID to mission state if not present", async () => {
      ...
      const updatedState = readMissionState(TEST_DIR)
    ...
    describe("Write/Edit tool direct work reminder", () => {
      const ORCHESTRATOR_SESSION = "orchestrator-write-test"
       ...
       beforeEach(() => {
         setupMessageStorage(ORCHESTRATOR_SESSION, "architect")
       })
  ...
  describe("session.idle handler (mission continuation)", () => {
    ...
    test("should not inject when main session is not in mission session_ids", async () => {
      // given - mission state exists but current (main) session is NOT in session_ids
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
      ...
      // then - should NOT call prompt because session is not part of this mission
      expect(mockInput._promptMock).not.toHaveBeenCalled()
    })

    test("should not inject when mission plan is complete", async () => {
    ...
    test("should clear abort state on message.updated", async () => {
      // given - mission with incomplete plan
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1")
    ...
    test("should include plan progress in continuation prompt", async () => {
      // given - mission state with specific progress
     ...
     test("should not inject when last agent does not match mission agent", async () => {
       // given - mission state with incomplete plan, but last agent does NOT match
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // given - last agent is NOT the mission agent
       cleanupMessageStorage(MAIN_SESSION_ID)
    ...
    test("should stop continuation after 2 consecutive prompt failures (issue #1355)", async () => {
      //#given - mission state with incomplete plan and prompt always fails
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    ...
    test("should cleanup on session.deleted", async () => {
      ...
      // Re-create mission after deletion
      writeMissionState(TEST_DIR, state)
