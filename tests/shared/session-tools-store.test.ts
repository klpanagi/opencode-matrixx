...
import { clearSessionTools, getSessionTools, setSessionTools } from "../../src/shared/session-state"
...
describe("session-tools-store", () => {
  ...
  test("stores and retrieves tools for a session", () => {
    ...
    const tools = { question: false, task: true }
    ...
    //#then
    expect(result).toEqual({ question: false, task: true })
  })
