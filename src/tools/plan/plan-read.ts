...
export function createPlanReadTool(ctx?: PluginContext): V2ToolDefinition {
  return {
    name: "plan_read",
    ...
    execute: async (args, context) => {
      try {
        ...
        if (JSON.stringify(result).length > MAX_PLAN_READ_RENDERED_BYTES) {
          return { content: await (JSON.stringify({ truncated: true, outline: buildOutline(lines), hint: TRUNCATION_HINT })) }
        }
        return { content: await (JSON.stringify(result)) }
      } catch (error) {
