...
function assertModelsResolvable(config: MatrixxConfig): void {
  const globalModel = config.global_model
  const unresolved: string[] = []

  for (const [name, entry] of Object.entries(config.agents ?? {})) {
    if (entry && !entry.disable && !entry.model && !globalModel) {
      unresolved.push(`agent "${name}"`)
    }
  }
  ...
  if (unresolved.length > 0) {
    throw new ConfigError(
      `Matrixx config error: no model resolved for ${unresolved.join(", ")}. ` +
        `Set an explicit "model" on the entry, define model_presets with an active_preset, ` +
        `or run "bunx opencode-matrixx setup" to generate a preset from your connected providers.`,
    )
  }
}
...
export async function loadPluginConfig(
  ...
  // Apply the active model preset (fill-in only — explicit `model` wins over
  // preset defaults; the preset only fills entries that remain without a model).
  config = applyActivePreset(config, config.active_preset)
  if (config.active_preset && config.model_presets?.[config.active_preset]) {
    log("Active model preset applied", { active_preset: config.active_preset });
  }

  assertModelsResolvable(config)
