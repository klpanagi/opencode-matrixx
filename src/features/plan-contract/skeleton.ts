/**
 * Plan Contract — Skeleton Generator
 *
 * Renders the canonical Oracle plan markdown skeleton. The eight H2 section
 * headings are emitted from CANONICAL_SECTIONS and the per-task subfield labels
 * from REQUIRED_TASK_SUBFIELDS, so a freshly generated plan can never drift from
 * the grammar that validatePlanContract enforces. The remaining guidance prose
 * (including the Agent-Executed QA scenario guidance) is static prompt content
 * preserved from the original Oracle template.
 */
import type { PlanSection, PlanTaskSubfield } from "./constants"
import { CANONICAL_SECTIONS, REQUIRED_TASK_SUBFIELDS } from "./constants"

/** Trailer appended to a generated task block, after the seven required subfields. */
const TASK_TRAILER = `  **Commit**: YES | NO (groups with N)
  - Message: \`type(scope): desc\`
  - Files: \`path/to/file\`
  - Pre-commit: \`test command\``

/** Agent-Executed QA scenario guidance embedded in the Verification Strategy section. */
const QA_SCENARIO_GUIDANCE = `### Agent-Executed QA Scenarios (MANDATORY — ALL tasks)
...
const TLDR_BODY = `> **Quick Summary**: [1-2 sentences capturing the core objective and approach]
> 
...
const CONTEXT_BODY = `### Original Request
...
const WORK_OBJECTIVES_BODY = `### Core Objective
...
- [Explicit exclusion from Seraph review]
- [AI slop pattern to avoid]
- [Scope boundary]`

const VERIFICATION_STRATEGY_BODY = `> **UNIVERSAL RULE: ZERO HUMAN INTERVENTION**
>
...
${QA_SCENARIO_GUIDANCE}`

const EXECUTION_STRATEGY_BODY = `### Parallel Execution Waves
...
|------|-------|-------------------|
| 1 | 1, 5 | task(category="...", load_skills=[...], run_in_background=true) |
| 2 | 2, 3, 6 | dispatch parallel after Wave 1 completes |
| 3 | 4 | final integration task |`

const COMMIT_STRATEGY_BODY = `| After Task | Message | Files | Verification |
|------------|---------|-------|--------------|
| 1 | \`type(scope): desc\` | file.ts | npm test |`

const SUCCESS_CRITERIA_BODY = `### Verification Commands
\`\`\`bash
...
const STATIC_SECTION_BODIES: Record<Exclude<PlanSection, "TODOs">, string> = {
  "TL;DR": TLDR_BODY,
  Context: CONTEXT_BODY,
  "Work Objectives": WORK_OBJECTIVES_BODY,
  "Verification Strategy (MANDATORY)": VERIFICATION_STRATEGY_BODY,
  "Execution Strategy": EXECUTION_STRATEGY_BODY,
  "Commit Strategy": COMMIT_STRATEGY_BODY,
  "Success Criteria": SUCCESS_CRITERIA_BODY,
}

const TASK_SUBFIELD_BODIES: Record<PlanTaskSubfield, string> = {
  "What to do": `  - [Clear implementation steps]
  - [Test cases to cover]`,
  "Must NOT do": `  - [Specific exclusions from guardrails]`,
  "Recommended Agent Profile": `  > Select category + skills based on task domain. Justify each choice.
  - **Category**: \`[construct | source | matrix-bend | quick | blue-pill | red-pill | writing]\`
  ...
  Parallelization: `  - **Can Run In Parallel**: YES | NO
...
const TODOS_INTRO = `> Implementation + Test = ONE Task. Never separate.
> EVERY task MUST have: Recommended Agent Profile + Parallelization info.`

function renderTaskSample(): string {
  const blocks = REQUIRED_TASK_SUBFIELDS.map(
    (label) => `  **${label}**:\n${TASK_SUBFIELD_BODIES[label]}`,
  )
  return `- [ ] 1. [Task Title]\n\n${blocks.join("\n\n")}\n\n${TASK_TRAILER}`
}

function renderTodosBody(): string {
  return `${TODOS_INTRO}\n\n${renderTaskSample()}`
}

function sectionBody(section: PlanSection): string {
  return section === "TODOs" ? renderTodosBody() : STATIC_SECTION_BODIES[section]
}
 ...
 */
export function renderPlanSkeleton(): string {
  const body = CANONICAL_SECTIONS.map((section, index) => {
    const separator = index === CANONICAL_SECTIONS.length - 1 ? "" : "\n\n---"
    return `## ${section}\n\n${sectionBody(section)}${separator}`
  }).join("\n\n")
  return `# {Plan Title}\n\n${body}`
}
